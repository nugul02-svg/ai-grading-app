...
def render_set(set_key, passage_html, q1_table_html, q1_labels,
    ...
    if current == 1:
        ...
        if submitted:
            ...
            with st.spinner("국어 선생님 AI가 답안을 꼼꼼히 읽고 있습니다..."):
...
"""
    ...
    # 2번 문항
        ...
        if submitted:
            ...
            with st.spinner("국어 선생님 AI가 문맥을 분석하고 있습니다..."):
                prompt = f"""
너는 다정하고 꼼꼼한 중학교 국어 교사야. 말투는 부드럽고 격려하는 구어체로 해줘.
[학생 답안] (1) {a1} / (2) {a2}
[채점 기준]
...
"""
    ...
    # 3번 문항
        ...
        if submitted:
            ...
            with st.spinner("국어 선생님 AI가 기획안 완성도를 체크하고 있습니다..."):
                prompt = f"""
너는 다정하고 꼼꼼한 중학교 국어 교사야. 말투는 부드럽고 격려하는 구어체로 해줘.
...
with tab1:
    ...
    render_set(
        ...
        q3_plan_html="<b>[영상 기획안]</b><br>◦ 주제: 사회적 촉진과 억제를 활용한 스마트한 공부법<br><br><b>[장면 1] 쉬운 과제를 할 때</b><br>◦ 시각 요소: 백색소음이 있는 밝은 도서관에서 친구들과 가볍게 미소 지으며 공부하는 학생들의 모습을 넓은 화면(풀샷)으로 보여줌.<br>◦ 청각 요소: 경쾌하고 리듬감 있는 배경음악과 함께 사람들의 가벼운 발소리와 책장 넘기는 소리를 깔아줌.<br><br><b>[장면 2] 어려운 과제를 할 때</b><br>◦ 시각 요소: ( Ⓐ )<br>◦ 청각 요소: ( Ⓑ )",
...
with tab2:
    ...
    render_set(
        set_key='set2',
        passage_html="[기자] 겨울철 불청객인 '정전기'란 정확히 무엇인지 설명 부탁드립니다.<br><br>[전문가] 정전기란 전하가 정지 상태로 있어 그 분포가 시간적으로 변화하지 않는 전기, 그리고 그로 인한 전기 현상을 말합니다. 쉽게 설명하면 흐르지 않고 머물러 있는 전기라고 해서 '움직이지 아니하여 조용하다.'는 뜻을 가진 한자 '정(靜)'을 써서 정전기라고 부르는 것이죠.<br><br>[기자] 우리가 실생활에서 쓰는 전기와는 어떻게 다른가요? 물에 비유해서 설명해 주시면 이해가 쉬울 것 같습니다.<br><br>[전문가] 아주 좋은 비유가 될 수 있습니다. 우리가 실생활에서 쓰는 전기가 '흐르는 물'이라면, 정전기는 '높은 곳에 고여 있는 물'이라고 할 수 있습니다.<br><br>[기자] 정전기가 일어날 때 찌릿한 느낌이 드는데, 혹시 위험하지는 않은가요?<br><br>[전문가] 정전기의 전압은 매우 높지만, 우리가 실생활에서 쓰는 전기와는 다르게 전하가 이동하지 않고 머물러 있어 위험하지는 않습니다. 어마어마하게 높은 곳에 고여 있는 물이지만 떨어지지 않고 있어서 별 피해가 없는 것과 같다고 이해하시면 됩니다.",
        q1_table_html="<table style='width:100%;text-align:center;border-collapse:collapse;margin-bottom:20px;'><tr style='background-color:#e2e8f0;'><th style='padding:15px;border:1px solid #cbd5e1;width:20%;'>대상</th><th style='padding:15px;border:1px solid #cbd5e1;width:25%;'>물의 상태에 비유</th><th style='padding:15px;border:1px solid #cbd5e1;width:25%;'>전하의 상태</th><th style='padding:15px;border:1px solid #cbd5e1;width:30%;'>위험성</th></tr><tr><td style='padding:15px;border:1px solid #cbd5e1;'>실생활 전기</td><td style='padding:15px;border:1px solid #cbd5e1;'>흐르는 물</td><td style='padding:15px;border:1px solid #cbd5e1;'>전하가 이동함</td><td style='padding:15px;border:1px solid #cbd5e1;'>감전 등의 위험이 있음</td></tr><tr><td style='padding:15px;border:1px solid #cbd5e1;'>정전기</td><td style='padding:15px;border:1px solid #cbd5e1;'>㉠</td><td style='padding:15px;border:1px solid #cbd5e1;'>㉡</td><td style='padding:15px;border:1px solid #cbd5e1;'>㉢</td></tr></table>",
        q1_labels=["(1) ㉠:", "(2) ㉡:", "(3) ㉢:"],
        q1_answer_keys=['q1_a', 'q1_b', 'q1_c'],
        ...
        q3_plan_html="<b>[영상 기획안]</b><br>◦ 주제: 전압은 높지만 위험하지 않은 정전기의 비밀<br><br><b>[장면 1] 실생활 전기 (흐르는 물)</b><br>◦ 시각 요소: 거대한 폭포수가 콸콸 쏟아져 내려오며 물레방아를 힘차게 돌리는 역동적인 그래픽을 보여줌.<br>◦ 청각 요소: 물이 거세게 부딪히는 웅장하고 큰 소리를 배경음으로 사용함.<br><br><b>[장면 2] 정전기 (고여 있는 물)</b><br>◦ 시각 요소: ( Ⓐ )<br>◦ 청각 요소: ( Ⓑ )",
...
with tab3:
    ...
    render_set(
        set_key='set3',
        passage_html="[기자] 최근 생성형 인공 지능이 그린 그림이 미술계에서 큰 화제를 모으고 있습니다. 어떤 작품인지 소개해 주실 수 있을까요?<br><br>[전문가] 네, 대표적으로 「에드몽 드 벨라미」라는 작품이 있습니다. 이 작품은 14~20세기에 그려진 초상화 1만 5,000점을 토대로 알고리즘과 데이터를 사용해 그려졌습니다. 뉴욕 크리스티 경매에서 최종 낙찰가 43만 2,000달러에 판매되어 큰 놀라움을 주었죠.<br><br>[기자] 그렇다면 이 그림을 인간이 만든 예술 작품과 같다고 볼 수 있을까요?<br><br>[전문가] 올림픽 경기를 예로 들어 볼게요. 우리가 올림픽에 열광하는 이유는 선수들이 경기를 위해 기울인 노력이나 열정을 알기 때문입니다. 반면 로봇이 한 번의 실수 없이 완벽하게 피겨 스케이팅을 해내더라도 우리의 마음을 울리지는 못하지요. 이처럼 인간의 작품에는 작가의 고유한 감정이나 철학, 그리고 작가가 살아온 삶의 경험, 세상을 바라보는 관점, 그를 둘러싼 환경 같은 내외부적인 요소가 종합적으로 담겨 있으므로 예술로 볼 수 있습니다. 하지만 인공 지능은 감정도 느끼지 못하고 독자적인 철학이나 이야기가 없기 때문에 이를 예술로 보기는 어렵습니다.<br><br>[기자] 그렇다면 인공 지능이 그린 그림은 가치가 전혀 없는 것인가요?<br><br>[전문가] 그렇지는 않습니다. 비록 인간과 같은 감정은 없더라도, 기존 미술계에 큰 변화를 가져왔다는 점에서 분명한 의미가 있습니다. 또한 앞으로 우리가 알고 있던 예술의 범주를 확장할 수 있다는 점에서 상징적인 가치를 지닙니다.",
        q1_table_html="<table style='width:100%;text-align:center;border-collapse:collapse;margin-bottom:20px;'><tr style='background-color:#e2e8f0;'><th style='padding:15px;border:1px solid #cbd5e1;width:15%;'>대상</th><th style='padding:15px;border:1px solid #cbd5e1;width:25%;'>올림픽 경기에 비유</th><th style='padding:15px;border:1px solid #cbd5e1;width:35%;'>예술로 볼 수 있는가<br>(근거 포함)</th><th style='padding:15px;border:1px solid #cbd5e1;width:25%;'>예술로서의 가치</th></tr><tr><td style='padding:15px;border:1px solid #cbd5e1;'>인간의 예술</td><td style='padding:15px;border:1px solid #cbd5e1;'>인간 선수의 노력과<br>열정이 담긴 경기</td><td style='padding:15px;border:1px solid #cbd5e1;'>작가의 경험, 관점, 환경이 담겨<br>있으므로 예술이다.</td><td style='padding:15px;border:1px solid #cbd5e1;'>감상자에게<br>남다른 감동을 줌</td></tr><tr><td style='padding:15px;border:1px solid #cbd5e1;'>인공 지능의 예술</td><td style='padding:15px;border:1px solid #cbd5e1;'>㉠</td><td style='padding:15px;border:1px solid #cbd5e1;'>㉡</td><td style='padding:15px;border:1px solid #cbd5e1;'>㉢</td></tr></table>",
        q1_labels=["(1) ㉠:", "(2) ㉡:", "(3) ㉢:"],
        q1_answer_keys=['q1_a', 'q1_b', 'q1_c'],
        ...
        q3_plan_html="<b>[영상 기획안]</b><br>◦ 주제: 인간의 감정이 담긴 진정한 예술의 가치<br><br><b>[장면 1] 감정이 없는 완벽한 기술</b><br>◦ 시각 요소: 로봇이 한 번의 실수 없이 완벽하게 피겨 스케이팅을 해내지만 우리의 마음을 울리지는 못하는 동영상을 보여줌.<br>◦ 청각 요소: 기계음이나 일정한 박자의 메트로놈 소리를 깔아 차갑고 정형화된 분위기를 조성함.<br><br><b>[장면 2] 마음에 울림을 주는 진정한 예술</b><br>◦ 시각 요소: ( Ⓐ )<br>◦ 청각 요소: ( Ⓑ )",
        q3_prompt_kwargs={'extra_criteria': "'작가의 감정, 경험, 철학', '마음을 울리는 감동' 등 지문을 근거로 효과 서술에 포함되어야 함."},
    )
