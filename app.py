...
# 변경 3·4: 첫 문장 및 영상 기획안 - 하늘색 상자 + 검은 글씨 (제시문과 동일 포맷)
...
with tab1:
    ...
    with st.form("form_set1"):
        ...
        s1_a1 = st.text_input("(1) ㉠:", placeholder="내용을 입력하세요.")
        s1_a2 = st.text_input("(2) ㉡:", placeholder="내용을 입력하세요.")
        s1_a3 = st.text_input("(3) ㉢:", placeholder="내용을 입력하세요.")
        ...
        # 변경 3: 첫 문장을 하늘색 상자 + 검은 글씨
        draw_blue_info_box("첫 문장: 과제의 특성과 난이도에 따라 우리의 학습 효율을 높이는 방법은 다르게 적용되어야 한다.")
        ...
        # 변경 4: 영상 기획안을 하늘색 상자 + 검은 글씨, (1)~(2)번으로 수정
        draw_blue_info_box(
            "<b>[영상 기획안]</b><br>"
        ...
        s1_vis = st.text_area("(1) Ⓐ 시각 요소 및 효과:", height=100, placeholder="시각 요소를 쓰고, 지문의 내용을 근거로 효과를 함께 서술하세요.")
        s1_aud = st.text_area("(2) Ⓑ 청각 요소 및 효과:", height=100, placeholder="청각 요소를 쓰고, 지문의 내용을 근거로 효과를 함께 서술하세요.")

        if st.form_submit_button("1세트 제출 및 AI 채점하기"):
            with st.spinner("국어 선생님 AI가 답안을 꼼꼼하게 읽고 있습니다..."):
                prompt = f"""
너는 다정하고 꼼꼼한 중학교 국어 교사야. 학생들이 작성한 국어 서논술형 답안을 채점 기준에 맞게 평가해줘.
말투는 "~했구나!", "~란다."와 같이 부드럽고 격려하는 구어체를 사용해줘.
...
with tab2:
    ...
    with st.form("form_set2"):
        ...
        s2_a1 = st.text_input("(1) ㉠:", placeholder="내용을 입력하세요.")
        s2_a2 = st.text_input("(2) ㉡:", placeholder="내용을 입력하세요.")
        s2_a3 = st.text_input("(3) ㉢:", placeholder="내용을 입력하세요.")

        st.markdown("---")
        st.markdown("[서·논술형 2] 윗글을 활용하여 '정전기의 특징'에 대한 설명문을 작성하려 한다. 주어진 첫 문장에 이어지는 내용을 &lt;조건&gt;에 맞추어 작성하시오.")
        draw_gray_box(cond_q2)
        draw_blue_info_box("첫 문장: 겨울철에 흔히 겪는 정전기는 우리가 평소 집에서 사용하는 전기와는 다른 뚜렷한 특징이 있다.")
        ...
        s2_vis = st.text_area("(1) Ⓐ 시각 요소 및 효과:", height=100, placeholder="시각 요소를 쓰고, 지문의 내용을 근거로 효과를 함께 서술하세요.")
        s2_aud = st.text_area("(2) Ⓑ 청각 요소 및 효과:", height=100, placeholder="청각 요소를 쓰고, 지문의 내용을 근거로 효과를 함께 서술하세요.")

        if st.form_submit_button("2세트 제출 및 AI 채점하기"):
            with st.spinner("국어 선생님 AI가 답안을 분석하고 있습니다..."):
                prompt = f"""
너는 중학교 국어 선생님이란다. 친절하고 명확하게 학생의 답안을 피드백해 주렴.
...
with tab3:
    ...
    with st.form("form_set3"):
        ...
        s3_a1 = st.text_input("(1) ㉠:", placeholder="내용을 입력하세요.")
        s3_a2 = st.text_input("(2) ㉡:", placeholder="내용을 입력하세요.")
        s3_a3 = st.text_input("(3) ㉢:", placeholder="내용을 입력하세요.")

        st.markdown("---")
        st.markdown("[서·논술형 2] 윗글을 활용하여 '인공 지능이 그린 그림을 바라보는 시각'에 대한 설명문을 작성하려 한다. 주어진 첫 문장에 이어지는 내용을 &lt;조건&gt;에 맞추어 작성하시오.")
        draw_gray_box(cond_q2)
        draw_blue_info_box("첫 문장: 인공 지능이 그린 그림이 늘어나는 요즘, 우리는 이 작품들을 어떤 눈으로 바라봐야 할지 올바르게 생각해야 한다.")
        ...
        s3_vis = st.text_area("(1) Ⓐ 시각 요소 및 효과:", height=100, placeholder="시각 요소를 쓰고, 지문의 내용을 근거로 효과를 함께 서술하세요.")
        s3_aud = st.text_area("(2) Ⓑ 청각 요소 및 효과:", height=100, placeholder="청각 요소를 쓰고, 지문의 내용을 근거로 효과를 함께 서술하세요.")
...
- 문항 1 (빈칸): ㉠={s3_a1}, ㉡={s3_a2}, ㉢={s3_a3}
  * 정답 기준: ㉠ 로봇이 실수 없이 완벽하게 피겨 스케이팅을 해내는 것, ㉡ 감정이나 철학/이야기가 없어 예술로 보기 어려움, ㉢ 미술계 변화 유발 및 예술 범주 확장이라는 상징적 가치.
- 문항 2 (설명문): (1) {s3_q2_1} / (2) {s3_q2_2}
  * 채점 기준: 괄호 안에 설명 방법 표기가 들어가야 하고 본문 텍스트 내용만을 활용해야 해.
- 문항 3 (영상기획): (1) {s3_vis} / (2) {s3_aud}
