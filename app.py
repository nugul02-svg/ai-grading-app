...
with tab1:
    ...
    render_set(
        ...
        q1_hints=["구(절) 단위로 쓰세요.", "'혼자 집중'이나 '차분하게 혼자' 명확히 서술하세요.", "'사회적 억제' 전체 명칭을 쓰세요."],
...
with tab2:
    ...
    render_set(
        ...
        q1_hints=["'높은 곳에 고여 있는 물' 전체를 명시하세요.", "'정지 상태이다', '이동하지 않는다' 등으로 구체적으로 쓰세요.", "'위험하지 않다' 혹은 '피해가 없다'로 정확히 작성하세요."],
...
with tab3:
    ...
    render_set(
        ...
        q1_hints=["로봇의 완벽한 비유 행동 전체를 구 단위로 적으세요.", "감정/철학이 없어 예술로 보기 어렵다는 내용을 정확히 서술하세요.", "변화나 범주 확장 같은 상징 가치를 서술하세요."],
