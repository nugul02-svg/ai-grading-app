...
def grade_set3(a1, a2, a3, q2_1, q2_2, vis, vis_eff, aud, aud_eff):
    fb = {"q1": [], "q2": grade_q2(q2_1, q2_2), "q3": []}
    if re.search(r'(로봇|기계)', a1) and re.search(r'(실수|완벽|피겨)', a1): fb["q1"].append("✅ ㉠ 정답")
    else: fb["q1"].append("❌ ㉠ 오답: '로봇이 완벽하게 피겨 스케이팅을 해내는 것'이 필요합니다.")
    if re.search(r'(감정|철학|이야기)', a2) and re.search(r'(없|어렵|아니)', a2): fb["q1"].append("✅ ㉡ 정답")
    else: fb["q1"].append("❌ ㉡ 오답: 감정이나 철학이 없어 예술로 보기 어렵다는 내용이 들어가야 합니다.")
    if re.search(r'(미술계|변화|범주|확장|상징적)', a3): fb["q1"].append("✅ ㉢ 정답")
    else: fb["q1"].append("❌ ㉢ 오답: 미술계 변화나 예술의 범주 확장이라는 가치가 명시되어야 합니다.")
...
cond_q3 = """
<ul style='margin-bottom: 0;'>
<li>🎯 윗글을 참고하여 특성이 잘 드러나도록 Ⓐ와 Ⓑ에 들어갈 연출 계획을 세울 것.</li>
...
with tab1:
    ...
    with st.form("form_set1"):
        ...
        st.markdown("---")
        st.markdown("#### [문항 3] 영상 매체 기획 (어려운 과제를 할 때)")
        draw_gray_box(cond_q3)
        s1_vis = st.text_area("Ⓐ 시각 요소:", height=68, placeholder="어떤 화면을 보여줄 것인가요?")
        s1_vis_eff = st.text_area("Ⓐ 시각 효과:", height=100, placeholder="지문의 내용을 근거로 효과를 상세히 적어주세요.")
        s1_aud = st.text_area("Ⓑ 청각 요소:", height=68, placeholder="어떤 소리를 들려줄 것인가요?")
        s1_aud_eff = st.text_area("Ⓑ 청각 효과:", height=100, placeholder="지문의 내용을 근거로 효과를 상세히 적어주세요.")
...
with tab2:
    ...
    with st.form("form_set2"):
        ...
        st.markdown("---")
        st.markdown("#### [문항 3] 영상 매체 기획 (정전기)")
        draw_gray_box(cond_q3)
        s2_vis = st.text_area("Ⓐ 시각 요소:", height=68)
        s2_vis_eff = st.text_area("Ⓐ 시각 효과:", height=100)
        s2_aud = st.text_area("Ⓑ 청각 요소:", height=68)
        s2_aud_eff = st.text_area("Ⓑ 청각 효과:", height=100)
...
with tab3:
    ...
    with st.form("form_set3"):
        ...
        st.markdown("---")
        st.markdown("#### [문항 3] 영상 매체 기획 (마음에 울림을 주는 진정한 예술)")
        draw_gray_box(cond_q3)
        s3_vis = st.text_area("Ⓐ 시각 요소:", height=68)
        s3_vis_eff = st.text_area("Ⓐ 시각 효과:", height=100)
        s3_aud = st.text_area("Ⓑ 청각 요소:", height=68)
        s3_aud_eff = st.text_area("Ⓑ 청각 효과:", height=100)
