...
def draw_side_guide():
    with st.sidebar:
        st.header("📖 개념 길잡이")import streamlit as st
import re
...
def draw_side_guide():
    with st.sidebar:
        ...
        with st.expander("📝 1. 설명 방법 공식과 예시", expanded=True):
            st.markdown("""
            * 분류와 구분: ~는 ~라는 기준에 따라 ~와(과) ~로 나뉜다.
              * 예: 악기는 소리 내는 방법에 따라 타악기, 현악기, 관악기 등으로 나뉘며, 드럼과 장구는 타악기로 묶이고 첼로와 비올라는 현악기로 묶인다.
            * 비교와 대조: [공통점] ~와 ~의 공통점은 ~이다. / [차이점] ~는 ~이지만, ~는 ~이다.
              * 예: 호랑이와 사자는 둘 다 고양잇과의 육식 동물이라는 공통점이 있지만, 호랑이는 단독 생활을 하고 사자는 무리 생활을 한다는 차이점이 있다.
            * 정의: ~란 ~를 말한다.
              * 예: 우정이란 친구 사이의 정을 말한다.
            * 인과: ~ 때문에 ~ 한다.
              * 예: 올해는 비가 거의 오지 않아서 흉년이 들었다.
            * 예시: 예를 들어 ~
              * 예: 콩을 원료로 만든 식품의 예로는 두부, 메주, 된장, 간장 등이 있다.
            * 분석: ~는 ~와(과) ~로 이루어져 있다.
              * 예: 곤충의 몸은 크게 머리, 가슴, 배의 세 부분으로 이루어져 있다.
            """)
